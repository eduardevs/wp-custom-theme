...
?>
        ...
        <input type="checkbox" value="3" name="<?= self::META_KEY ?>" <?php checked($value, '1') ?>>
...
<?php
    ...
    public static function save(int $post)
    {
        ...
        // die();
        if (
            array_key_exists(self::META_KEY, $_POST) && current_user_can('publish_posts', $post) && wp_verify_nonce($_POST[self::NONCE], self::NONCE)
        ) {
